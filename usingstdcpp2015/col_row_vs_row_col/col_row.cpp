...
{
  if (argc < 3) {
    std::cout << "Usage: ./col_row ${data_size} ${repeat_time}\n";
    return 0;
  }
  ...
  std::cout<<"n;col_row"<<std::endl;
    
  for(std::size_t i=0;i<repeat_time;i+=1) {
    std::size_t               m=static_cast<std::size_t>(std::sqrt(data_size));
